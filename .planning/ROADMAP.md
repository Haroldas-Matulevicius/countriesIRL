...
- [x] **DONE 2026-08-07** (`3f41b19`, `3803271`, `75ef73c`) — the composite frame built once and inspected in bytes (IHDR 1080×1080): ocean `#F5EFE6` α255 · Libya `#E5E7EB` · Brazil/Kazakhstan/India `#FCAE91`/`#A50F15`/`#2171B5` · coastline band **0** dark px vs Rhine band **85** · bottom band 238.485/235.209/231.004 against a bands-off column flat at 230.864 · title box **4,188** ink → **0** cleared · legend box **6,981** ink, unchanged by clearing the title · **zero** ramp pixels in the top band and title box against a positive control of 941/1,119/1,104. **The frame runs on `Warm paper`, not the reference's white water** — a band on white fades 239.626 to 239.626, so a band claim sampled there passes while measuring nothing. **The blank-composition RED proof reddened six of seven properties** — the coastline/interior-border pair survives it, because a reloaded blank is still a rendered world map at the shipped defaults; recorded plainly rather than claimed as a pass. **The plan's uncoloured-fill claim as written could not fail** (a blank map is entirely uncoloured fill), so a third conjunct was added. **Wave 0 row 7 — the `package.json`-unchanged assertion — had never landed** and is now a gate rather than a box. ~~`04-15-PLAN.md`~~ — Composition export integration: the reference frame in real PNG bytes, no baseline image *(wave 12)*
- [x] **DONE 2026-08-07** (`0653333`, `d616463`) — **the phase's honest close.** A **byte-level range diff** against the phase-start SHA `0df7fff9d1060e6ab3efa5aacdb8c3228a88b7cb` proves **zero new npm packages**, **RED-proved against a COMMITTED `left-pad`** — the shape `CLAUDE.md` records `git diff --quiet HEAD` as *passing silently on*, reproduced here for contrast before the new gate went red — plus a second proof on its own anti-vacuity floor (an empty range reddens). The **independent non-author review** re-ran all six gates with real output (lint clean · **875/875** unit · build clean · **138/138** Chrome e2e · `data:world:check` PASS · ceiling **337**; nothing below baseline), **re-performed six RED proofs across the phase and all six reproduced**, hunted eleven recorded failure shapes, and addressed Live Invariants 1–10 and Immutable Safety Constraints 1–10 individually. **No overclaim was found in any SUMMARY's headline result.** **Five findings, NONE fixed** (a reviewer who fixes is no longer independent) — the sharpest being a **raw NUL byte** that makes git classify `compositionText.test.ts` **binary**, so its 333 lines were **invisible in the aggregate diff** and `grep -I` silently skips it. ⛔ **All EIGHT physical checks recorded `NOT PERFORMED`** — the blanket sight-unseen authorization converts to *proceeding past* the gate, **never to a pass on a cell**. ~~`04-16-PLAN.md`~~ — Independent non-author review, the dependency gate, and the eight physical checks *(wave 13, owner gate)*
...
8. ~~**Run `/gsd:plan-phase 4`**~~ ~~**Run `/gsd:execute-phase 4`**~~ **BOTH DONE 2026-08-07.**
   Phase 4 shipped **at the code level only** — 16/16 plans, closed by an independent non-author
   review. ⛔ **Nobody has looked at the result: all eight physical checks are `NOT PERFORMED`.**
   ▶ Next: hand
   [`04-ACCEPTANCE.md`](phases/04-visual-cartographic-system-1-5-2-weeks/04-ACCEPTANCE.md) to the
   owner — the eight cells, and **`U-6` first**, since it is the one place the phase knowingly
   departs from the owner's own reference image. Then work the five findings in
   [`04-16-REVIEW.md`](phases/04-visual-cartographic-system-1-5-2-weeks/04-16-REVIEW.md) § 9.
9. **Decide `OQ-2` before planning Phase 5.** The rail-height floor is **552px** (measured by
   `04-01`, not the 540px the spec estimated), and **`05-05`'s Data HUD would add an eighth row
   at ~600px.** Planning Phase 5 without settling this schedules a known overflow.
   **This is now the single blocking item before `/gsd-plan-phase 5`.**
10. **Phase 4 was CLOSED by the owner on a free exploration (2026-08-07), not on the structured
    gate.** Six of its eight cells were never performed and **cannot be inherited** — they join
    Phase 3's nine. Phase 6's `06-03` WCAG audit and the v1.1 acceptance matrix are where they
    finally have to be done. **`U-6` is still unreviewed** and is the one place the product
    knowingly departs from the owner's own reference image.
11. **`F-7` is open:** the legend bar is still slightly off and collides with the boxes — the
    owner's own report, which **answers `OQ-3` in the negative**. `G-1` is worked but **not
    closed**. Fixing it **moves exported pixels** (D-25) and needs itemised re-baselining.
12. **The legend still carries the defect `F-6` fixed for composition text.**
    `LEGEND_CHARACTERS_PER_LINE` derives from the same worst-case-uniform model, so its "a full
    line of the widest character cannot overflow" claim **does not hold for latin-ext labels**
    (`U+01F1 DZ` is 1.3745em vs `W`'s 1.0202). `F-1` — whether **14** is the right legend bound —
    remains **unvalidated**. Migrating the legend to `src/utils/interMetrics.ts` closes both.
...
| 4 | Visual & Cartographic System | ✅ **COMPLETE** 2026-08-07 — **closed by the OWNER on a free exploration**, exactly as Phase 3 was closed. The owner exercised PNG export, colours and ramp shading, the title field, and legend-entry renaming: *"everything seemed to work decently."* **That closes the phase; it does NOT fill the cells.** Execution had run under a **blanket, in-advance, sight-unseen proceed-authorization** ([`04-AUTHORIZATION.md`](phases/04-visual-cartographic-system-1-5-2-weeks/04-AUTHORIZATION.md), written **before** execution) which **authorized proceeding** and was **not a content review** | 16/16 | ⚠️ **SIX of the eight acceptance cells were NEVER PERFORMED and still are not** — A9 screen-reader · A10 physical 200% zoom · A11 dark-theme review · A12 latin-ext diacritic export · A13 the rail at ≥1200px (D-5) · cartographic resemblance. **Skipped is not passed; none may be cited as verified, and none may cite a Phase 3 result.** **Cell 8 (PNG-vs-screen) PASSED** on the owner's own comparison. **Cell 6 (G-3 rework) is PARTIAL ONLY** — the panel drew no complaint but the owner did not re-judge the three original complaints, so **`G-3` is NOT resolved** → [`04-ACCEPTANCE.md`](phases/04-visual-cartographic-system-1-5-2-weeks/04-ACCEPTANCE.md). **The acceptance session found TWO defects.** ✅ **`F-6` — the title field refused text far too early (*"28 is little"*), CONFIRMED, ROOT-CAUSED, and FIXED.** The fit rule counted characters against a **worst-case-uniform** bound charging every character the advance of `W`, roughly **halving** real capacity (a `medium` title got **22** characters; a 45-character title really renders at **970 of 1016** units) — the over-estimate `03-VERIFICATION.md` predicted (~1.8×) and never had rebutted. It was **also NOT conservative**: since `04-04`'s latin-ext face, `U+01F1 DZ` is **1.3745em**, 35% wider than `W`, so 22 of them sat **on** the old bound while rendering **46% past the line** — **the old rule accepted a string that clips.** `src/utils/interMetrics.ts` now vendors **real measured advances plus a pair-kern table** (installed Chrome, canvas `measureText`, vendored woff2; the harness reproduced `W@600 = 1.0202` exactly), returning a **provable upper bound**. ⚠ **This does NOT close `A12`** — it changed how text is *measured*, not whether latin-ext glyphs *render* correctly in a PNG. ⏳ **`F-7` — the legend bar is still slightly off and collides with the boxes. OPEN**, and it **answers `OQ-3` in the NEGATIVE**, so **`G-1` is worked but NOT closed.** **Gate after the fix:** lint clean · **886/886** unit (48 files) · build clean · **138/138** Playwright on Chrome 151.0.7922.76 · `data:world:check` PASS (248 units, **195 core states**, **207 colourable units**, mesh 327 geometries / 366,767 B) · selector ceiling **337** · **zero new npm packages**. **Review findings: FOUR of five now fixed** — `F-1`/`F-2` (raw NUL bytes) at `9b88e67`, `F-3` (the void unquoted-grep proof; **the invariant itself re-verified INTACT**, all ten hits classified, none a render or export path) and `F-4` (stale `config.json`) at `6534ff0`; `F-5` stays informational → [`04-16-REVIEW.md`](phases/04-visual-cartographic-system-1-5-2-weeks/04-16-REVIEW.md). ⚠ **`U-6` STILL SHIPS UNREVIEWED** — single composition ink `#111827`, **the one place the phase knowingly departs from the owner's Eurostat reference**; an **`[ASSUMED]` row, never an owner decision.** ⚠ **Browser scope drifted mid-phase and BOTH are on the record:** `04-01`…`04-06` on Chrome 151.0.7922.75, `04-07`…`04-16` and the review on **151.0.7922.76**. **Edge is NOT installed and NOT certified**; Firefox and Safari have never been run here. **Still open:** **`OQ-2` blocks Phase 5 planning — the rail floor is 552px, not 540px, and `05-05`'s Data HUD adds an eighth row (~600px)** · `OQ-1`, `OQ-4`, `OQ-5` · `F-1` (whether 14 is the right **legend** bound) still **unvalidated**, and **the legend still derives from the same worst-case model `F-6` replaced, so its "a full line of the widest character cannot overflow" claim does NOT hold for latin-ext labels** · storage headroom **48% → 17%**. **`G-2` is CORRECTED, not merely closed** — a **`medium`-size trap**, not a flat 15–32 char range; `03-UAT.md` **annotated, never rewritten**. ⚠ **The decision-coverage gate did NOT pass — it could not run** (`could-not-parse`); recorded **INCONCLUSIVE**, coverage established independently **three** times, **18/18**. Sequential ramps, water presets, interior-border mesh, gradient bands, text tools, legend overhaul, latin-ext coverage, V3 persistence. **3 one-way decisions landed** (D4-10, D4-11, D4-17) — **saved compositions change appearance on load.** → [`04-RESEARCH.md`](phases/04-visual-cartographic-system-1-5-2-weeks/04-RESEARCH.md) · [`04-UI-SPEC.md`](phases/04-visual-cartographic-system-1-5-2-weeks/04-UI-SPEC.md) · [`04-VALIDATION.md`](phases/04-visual-cartographic-system-1-5-2-weeks/04-VALIDATION.md) |
