...
*Last updated: 2026-08-06 (Phase 4) — **Live Invariant 5 amended, not deleted** (D4-10, plan `04-03`). Its intent survives: the catalog is still whole and still unfiltered, and out-of-scene rows are still *disabled* rather than removed. Its text stopped being true once the colourable count moved, so it now names both numbers and what each counts — 195 core states (definition unchanged) and 207 colourable units, never interchangeable. The twelve `self-colorable` units are the difference. Detail in [`data.md`](data.md) § Three colour policies, two counts. Nothing in §Immutable Safety Constraints moved: no geometry was promoted and no approval was implicated.*
