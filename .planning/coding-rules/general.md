...
| 8 | ~~**Legend opacity is a single 0–100 scale.** A stored 0–1 fraction is repaired to percent and reported, not silently clamped.~~ **RETIRED 2026-08-07 (D4-11, plan `04-12`) — the field it governed no longer exists.** `LegendState.backgroundOpacity` was deleted with `theme` and `borderStyle` when the legend lost its box chrome, so there is no scale left to disagree about and no repair left to report. It is recorded here rather than deleted, because an invariant that can no longer be violated is **retired**, never silently removed: a reader who finds the row missing cannot tell whether it was dropped or was never true. **What replaced it** is the opposite discipline, and it is live: a stored V2 record still *carries* the three deleted fields, and their presence must NOT be reported as corruption — see [`storage.md`](storage.md) § A removed field is not a damaged one. | [`storage.md`](storage.md) |
