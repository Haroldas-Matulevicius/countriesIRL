...
**The canonical child order of `svg.map-canvas`** (`04-UI-SPEC.md § 6.5`; rows marked ○ do not
exist yet and are named so later plans do not renegotiate the order):

```
svg.map-canvas  (viewBox "0 0 1080 1080")
├── rect[data-layer="surface"]     ← water; inline fill; OUTSIDE the camera   (04-01) ●
├── defs[data-layer="paint"]       ← band gradients; inline literal stops     (04-10) ●
├── g[data-layer="camera"]         ← MUST stay before the legend
│   ├── g[data-layer="outgoing-scenes"]
│   ├── g[data-layer="countries"]
│   ├── g[data-layer="borders"]    ← interior mesh; non-interactive           (04-09) ●
│   └── g[data-layer="highlight"]  ← data-editor-only; hover / selection      (04-09) ●
├── g[data-layer="bands"]          ← OUTSIDE the camera; before the legend    (04-10) ●
├── g[data-layer="legend"]
├── g[data-layer="text"]           ← OUTSIDE the camera; title/subtitle/attrib (04-11) ●
└── g[data-layer="band-handles"]   ← data-editor-only; the resize affordances (04-10) ●
```

**The stack is now COMPLETE — every row is ●.** `04-11` closed the last one, and U-8's paint
order (bands → legend → text) is asserted three ways: as clone child order in `export.test.ts`
×2 and `export.spec.ts` ×1, and on real pixels at a forced overlap. A new layer added later joins
all four or one of them fails.

### The gradient bands, and the id that keeps them alive (D4-16, plan `04-10`)

**A referenced `id` is PAINT. An unreferenced one is editor semantics.** `sanitizeExportClone`
strips every `id` unless `collectReferencedIds` found a `url(#…)` or `href="#…"` pointing at it, so
a `<linearGradient>` is kept alive by the band rect's `fill="url(#…)"` and by nothing else. Get that
wrong and the failure is **silent**: the `<defs>` subtree is still in the clone, the markup still
reads correctly, and the reference resolves to nothing inside the isolated export document — the
editor keeps its band and the download loses it.

Three consequences, all now gated:

- **A gate must not assert `clone.ids === 0`.** That is what a stripped gradient looks like, so the
  assertion confirms the break instead of catching it (`coding-rules/export.md` said so before this
  plan; `04-10` is the plan that made it bite). The replacement asserts three things: no surviving
  id is unreferenced, no surviving reference dangles, and at least one reference exists — the last
  because the first two are vacuous at zero.
- **Measured evidence that the strip rule discriminates:** with the top band on and the bottom band
  off, the sanitized clone carries **one** id, not two. The bottom gradient has no rect pointing at
  it and is correctly stripped.
- **The stops are inline LITERALS.** `stop-color` and `stop-opacity` carry resolved values derived
  from `settings.surfaceColor`. Not a `var()`, for the reason the table above gives. Not a subtree
  `<style>` either: that route works, but it puts a second `<style>` beside the injected font one,
  which the clone contract discusses by name.

**A band is invisible on white water, BY DESIGN — and that shapes every gate about it.** It fades
from the surface colour to transparent, so over water it fades from the water colour *to the water
colour*. Measured: on `Warm paper` (`#F5EFE6`) water, a band column over open ocean reads 239.626 at
every row with the band on and with it off. **The signal only exists where the band overlays
something else.** So a band gate needs a non-white surface *and* a sample column that crosses land,
and it owes an assertion that its samples ARE land — a column that drifts onto ocean otherwise
measures nothing and passes.

**Removal and inversion are two failure modes and need two probes.** A single "band pixels are
lighter" check passes on an upside-down gradient, because some band pixels are still lighter. The
shape that works:

| Probe | What it asserts | Reddens on removal | Reddens on inversion |
|---|---|---|---|
| presence | the column's total deviation from a bands-off control | **yes** (0) | no (it grows) |
| ordering | near > mid > far, pairwise, plus a near-to-far floor | yes | **yes** |

**Order matters: presence runs FIRST.** A deleted band flattens the column, which trips the ordering
assertion and reports an *inversion* — one mutation reddening a claim it is not about. Running
presence first makes each message name its own failure. And because presence stays green through the
inversion (measured 18.946 against 5.626 right-way-up), the two are demonstrably separate claims
rather than one claim written twice.

**Composition geometry is in user units, and a "44px hit area" is not.** At 1280 × 720 the
full-bleed canvas renders the 1080 square at 720 CSS px, so a 44-**unit** rect measures **29.3 CSS
px**. The band handle is 88 units instead, derived as `44 / 0.5` where 0.5 is the scale at the 540px
canvas floor. **It is a fixed number rather than a measured one on purpose:** sizing it from the
rendered scale needs a resize observer inside `MapCanvas`, and `uiContract.test.ts` keeps that class
of observer out of that file so a rail or panel reflow can never re-derive projection, camera, or
export geometry. An affordance is not worth an exception to that invariant.

**`resolveBandExtents` is the ONE reader of `visible ? height : 0`.** `MapCanvas` places the rects
from it and `04-12`'s band-aware legend inset consumes it. Two copies of that expression is how the
legend ends up sitting under a band the creator has grown.

**When a new layer contaminates an existing sample, MOVE THE SAMPLE.** The top band paints the
surface colour at full opacity along `y = 0`, so `04-09`'s four-corner land-versus-water
discrimination started measuring the band: the top corners read (254, 254, 254). Re-baselining them
would have kept the test green and killed the claim, because a banded corner reads the water colour
whatever is under it. The rows moved past `BAND_MAX_HEIGHT` — structurally out of reach of any band
the product can draw, including one dragged to the cap — with the structural half asserted and the
exact rows measured, because the two nearest structural candidates are contaminated by geography.

**Two of those rows landed in `04-09`, and each carries a rule worth stating on its own.**

**`g[data-layer="borders"]` — the interior mesh.** It draws `04-06`'s derived
`world-borders-modern.geojson`, the edges present in exactly two polygons, so a coastline is
absent from it *by construction*. Four properties, each with a recorded reason:

- **Inside the camera, after the countries.** Inside, or the lines detach from the geometry they
  describe the moment a creator pans; after, or the fills paint over them.
- **Its own class, `border-mesh-path` — never `scene-path` or `country-path`.**
  `sanitizeExportClone` normalises those two against the **coastline** contract
  (`data-coastline-weight`), so a mesh it matched would be re-stroked to the coastline weight in
  the download and **deleted outright at `coastlineWeight: none`** — the editor-versus-download
  disagreement the whole phase exists to close. Its stroke rides as inline attributes instead, so
  the exporter has nothing to do for it and cannot do the wrong thing.
- **Non-interactive as ATTRIBUTES** (`pointer-events="none"`, `aria-hidden="true"`), not as a
  rule in `MapCanvas.css`: this layer is in the export clone, and a rule reaches the editor and
  not the clone. `fill="none"` and the round joins are inherited presentation attributes on the
  group, set once.
- **Date-line wrapped on the SAME `WRAP_OFFSETS` array the polygons use.** Reusing the array is
  the point. The gate compares the mesh's distinct `transform` set against the polygons' own,
  so an offset added to or removed from the array moves both sides together, with a literal `3`
  beside it so the comparison is not satisfiable at zero.

**`g[data-layer="highlight"]` — hover and selection, and the rule that generalises from it:
anything editor-only carries `data-editor-only`, and the sanitizer removes it wholesale.** That
is what makes "interaction state never reaches the PNG" **structural** rather than a
neutralisation applied after the fact. It also **decouples** feedback from composition: `04-08`
made `coastlineWeight: none` the default, and the old feedback was a heavier stroke on the
country path — so at the shipped default there was no stroke left to thicken and a coastal
country produced no feedback at all. The country path's stroke is now decided by the creator's
`coastlineWeight` and by nothing else.

**A shared line layer cannot carry per-country state (CD-11).** A mesh segment is the boundary
*between two countries*, so weighting one segment highlights both. `ROADMAP.md § Phase 4 04-05`
claimed the weight states were re-expressed on the mesh; that line was amended by `04-09`. The
same reasoning applies to anything else derived from shared edges.

**An editor-only layer still needs ONE inline paint attribute, and the reason is a gate that
could not fail.** The highlight's stroke *colour* is a `var(--map-border-hover)` /
`--map-border-selected` rule — the mode-invariant tokens, which is the standing rule for a
selection stroke on map geometry, and their only consumer. But the export clone rasterises with
no stylesheet, so with the colour coming *only* from CSS a surviving highlight path rendered
**nothing** in the PNG — and the gate that deletes `data-editor-only` to prove the guarantee
**measured 0 either way**. A second, accidental mechanism was hiding the one under test. The
colour is therefore ALSO written inline from `SELECTED_BORDER_COLOR` / `HOVERED_BORDER_COLOR`,
the class rule out-specifies it on screen so the tokens still decide what the editor draws, and
`uiContract.test.ts` asserts each constant equals its token in both directions. With the inline
copy the same mutation measures **132** ink pixels.
...
### Creator text in the composition (D4-15, plan `04-11`)

**Creator text is the largest untrusted-input surface in the phase, and the mitigation is
structural rather than an escape function.** The value is set as `<text>` **children**, so React
can only ever produce a text node and `XMLSerializer` escapes `<`, `>`, and `&` on the way out.
`grep -cE "innerHTML|dangerouslySetInnerHTML|insertAdjacentHTML"` over `MapCanvas.tsx` returns
**0** and is an acceptance criterion, not a style note.

**`sanitizeCompositionText` deliberately does NOT escape.** It strips control and format
characters (`Cc`/`Cf`/`Zl`/`Zp` — `Cf` is where the bidi overrides live) and bounds the length in
**code points**, and that is all. Pre-escaping plus serializer escaping is double-escaping, which
puts a literal `&amp;` in the PNG. Sanitisation happens at the composition reducer's boundary,
beside `canonicalizeSurfaceColor`, so no render site has to remember it.

**Refusal, never truncation, and ONE derivation feeds both halves.** The fit rule decides the
counter's `--destructive` state *and* the export refusal, so the number a creator watches climb can
never disagree with the rule that blocks the export. The length cap (100 code points) is
deliberately **above** where text stops fitting: a creator has to be able to overshoot and be told
what to shorten, and a cap at the fit line would silently clip.

### Measure text; never count characters against a worst-case width

**This is the rule, and it was learned the expensive way.** Until 2026-08-07 the fit rule asked
*"how many characters of THE WIDEST KIND fit on this line?"* — a count against a bound derived from
one constant, `WIDEST_CHARACTER_ADVANCE_EM` (`W` at **1.0202em**). It was wrong in **both**
directions at once, which is why "just raise the limit" would not have fixed it:

- **Far too strict for real text.** Charging every character the price of the widest one roughly
  **halved** capacity: a `medium` title got **22** characters, while `'Countries I have visited
  across all of Europe'` (45 characters) really renders at **970 of the 1016** available units.
  `03-VERIFICATION.md` predicted this over-estimate (~1.8×) against the legend's own ceiling and
  its three grounds were never rebutted; the owner then hit it on the title in Phase 4 acceptance.
- **Not actually conservative.** `W` stopped being the widest glyph the moment `04-04` vendored the
  latin-ext face: `U+01F1 DZ` is **1.3745em**, 35 % wider. Twenty-two of them sat *on* the old
  bound, so the old rule **accepted a string that renders 46 % past the line.** A worst-case model
  is only safe while its worst case is still true, and **nobody re-derived it when coverage grew.**

**`src/utils/interMetrics.ts` is the single home** for real per-character advances and pair
kerning, measured by the same method that produced `1.0202` (installed Chrome, canvas
`measureText`, against the **vendored woff2 bytes**). The harness reproduced `W@600 = 1.0202`
exactly — *that* is what licenses trusting the rest of the table, and any re-measurement must
reproduce it too.

**`measureTextEm` returns a provable UPPER bound, and every rounding decision leans wide.**
Advances and kerns are stored **rounded up**; codepoints outside the vendored subsets cost
`WIDEST_KNOWN_ADVANCE_EM`; pairs below the tabulation threshold cost the **measured maximum**
untabulated kern. Under-stating is the clipped-PNG defect this repo has already shipped once.

⚠ **Summing advances is NOT automatically safe — some pairs kern APART.** But the fix for that has
its own trap, and it was caught by an e2e rather than by reasoning: charging the single worst pair
(`ïï`, `0.166em`) to every gap is provably safe and **far too blunt**, and it falsely refused
`'W'.repeat(22)` — a title that genuinely measures **1008 of 1016** units. `W`+`W` kerns by
`0.023em`. **A safe bound that is not also tight is a bug with a proof attached.** Hence a real
pair table for anything above the threshold.

⚠ **Vitest runs on `node`, and that is not a reason to count characters.** The constraint is met by
vendoring the measurement **as committed data**, so it is deterministic, identical in node and the
browser, and needs no DOM.

⚠ **The LEGEND still has the old defect.** `LEGEND_CHARACTERS_PER_LINE` derives from the same
worst-case model, so its *"a full line of the widest common character cannot overflow"* claim
**does not hold for latin-ext labels**, and `F-1` — whether **14** is the right bound — remains
**unvalidated**. Migrating it to `interMetrics` closes both. Recorded, not silently accepted.

**A gate whose subject is a fit rule must be built from something other than `W`.** Replacing the
rule wholesale broke **nothing** in the 875-test suite, because every pre-existing refusal test
built its subject with `'W'.repeat(...)` — the one character where the old and new rules agree by
construction. **Green was the warning, not the reassurance.** Assert *opposite* directions of the
defect (text wrongly refused must now pass; latin-ext digraphs wrongly accepted must now refuse),
so a merely looser rule fails half the gates. Related, still true: a test that tried to *recover*
the advance from `LEGEND_CHARACTERS_PER_LINE` went green against an advance of `1`, because
`floor()` eats the difference at all three legend sizes.

**An empty field renders NO element, and that is behaviour, not tidiness.** An empty `<text>`
still carries a `font-family`, and `collectCompositionFonts` would report it — so a composition
with no type at all would embed a font. `resolveCompositionTextLines` returns no line for an empty
field, which makes the decision a pure `node`-testable property instead of a JSX conditional
nobody can assert. The gate proving this is paired with its **counterfactual**: an empty `<text>`
that DOES declare the family is shown to register one, so the claim is not vacuously true of any
empty SVG.

**Text baselines are anchored to the SQUARE's edge by `TEXT_SAFE_INSET`, not to a band extent.**
A band is a backdrop for type, not a container. At the shipped defaults the title and subtitle sit
inside the top band and the attribution inside the bottom one, but a creator who drags a band shut
does not have their title follow it off the square — and no text pixel gate becomes a function of
band height. `resolveBandExtents` stays `04-12`'s legend seam. `TEXT_SAFE_INSET` **is**
`LEGEND_SAFE_INSET`, imported: one inset for every composition edge is what puts the title, the
legend, and the attribution on the same left rule.

**One composition ink (U-6), and the arithmetic is why.** `COMPOSITION_INK_COLOR` `#111827`
(L 0.0091) needs a surface at `L >= 0.2164`; a second grey `#4B5563` (L 0.0889) would need
`L >= 0.575` — near-white water only, retiring three of the four shipped presets. Hierarchy is
carried by size and weight. ⚠ **This knowingly departs from the owner's reference image and the
owner has NOT reviewed it**; the authorization in force is a blanket, in-advance, sight-unseen
proceed-authorization (ISC 8). Flagged for `04-ACCEPTANCE.md`.

**One font stack, in one place.** `COMPOSITION_FONT_FAMILY` lives in `interFontFace.ts`;
`LegendOverlay` aliases it and the text layer imports it. Two stacks that must agree is how the
legend and the title end up in different typefaces in the same PNG. **No font picker ships**
(U-9) — if one ever does, its options must derive from `EXPORT_FONT_FACE_BUILDERS.keys()`, because
a family the registry does not know falls back **silently**.

**PRESENCE FIRST, THEN CORRECTNESS, for every gate about text.** A gate that samples "is there ink
here" goes green for text rendered in the wrong font, at the wrong size, or showing the wrong
string. Every pixel claim in `describe("composition text")` is preceded by DOM assertions on the
element's existence, its text, its size, weight, anchor, ink literal, and family — and the forced
overlap adds a pixel presence probe ahead of both ordering claims, so text that never reached the
frame cannot redden a claim about paint order.

**A derived crop must be bounded to the 1080 frame.** This is recorded because a RED proof defeated
the gates without it: moving the text outside the viewBox moves the *derived* crop with it, and
`drawImage` from a source rect off the bitmap yields **transparent black**, which every ink counter
in the suite reads as solid ink. The content floors passed on 28,050 phantom pixels while the exact
defect they exist to catch had happened.
...
## The Flat Panel Vocabulary (Phase 4, D4-04 / D4-05, plan 04-07)

**One card per panel became ZERO cards per panel, and the third of the owner's
three `G-3` complaints was a contract violation rather than a preference.**
`Design.md § 9` says *"Don't put a border on top of a border. Stack by background
shift (Platinum → Porcelain)."* The Colors panel was a Porcelain card holding
hairlined Platinum tiles beside a bordered input — a border inside a border
inside a panel. `03-04` moved the system to flat hairline elevation and `03-07`
migrated the panel without a layout redesign, so *"hate the multi boxes within"*
was a bug report.

**A section is type plus a rule, never a box.** A `--text-body-sm` weight-500
label, its content, and a `border-block-start` hairline on the **next** section,
with `--space-xl` on each side of the rule. Elevation runs Platinum → Porcelain,
**one step, never two**: only leaf interactive surfaces (rows, tiles, pills,
inputs) are Porcelain, and the panel body stays Platinum.

**The shared vocabulary lives in `editor.css` as `.panel-*`, not in a surface
file.** `04-UI-SPEC.md § 11` rule 1: *"A `Map style` pill that is a copy of a
Colors pill is a defect, not a convenience."* `04-01` authored the vocabulary
privately in `mapStyle.css` because it shipped first; `04-07` moved it to the
container that places both panels — the same `03-10` precedent that homed the
section card in `editor.css` rather than duplicating it across three surfaces.
`mapStyle.css` is down to one rule: `.map-style__custom`, a sub-block *inside*
the Water group that no sibling-divider rule can reach.

**`<fieldset>` survives as SEMANTIC grouping only.** `border: 0; padding: 0;
margin: 0`, its `<legend>` is the section label, and it carries `disabled` for
the whole group at 40% opacity with no pointer events. **A disabled group is
`<fieldset disabled>`, never `aria-disabled` on a still-clickable control** — one
announces a thing the other does not do, and the native attribute removes the
tab stop and the click together. Measured consequence, asserted in
`colorsPanel.spec.ts`: with nothing selected the Colors panel contributes **zero**
tab stops, and the spec asserts those absences rather than only what is present.

**One `<h2>` per panel and it is the panel title.** A second heading inside a
surface already titled `Colors` was the information-architecture half of `G-3`.
Section labels become their group's accessible name — natively via `<legend>`,
or via `aria-labelledby` on a `<section>`. The empty state keeps a **real
heading** at `<h3>`: it answers "what do I do now" and a screen-reader user
navigating by heading has to find it.

**A field named by its section label needs the `aria-labelledby` reference, and
the reference is a gate.** The hex input prints no second `Custom color` at
328px of content; a broken id renders identically and silently un-names the
field, so `ColorPicker.test.tsx` resolves the id rather than trusting it.

**Every width claim derives from 328px, not 360.** The open flyout is
`--panel-width-open: 360px`, declared **once** in `editor.css` (never
`theme.css` — that allowlist governs `theme.css` only, and Phase 4 adds no theme
tokens) and read by three consumers, one of which is a *derived* cap. It is a
named token because `editor.css` already spells `360px` for the compact sheet's
height cap and for the narrowest contained viewport: after D4-05 the number
means three things in one file. **The widening buys icon-only rows, single-word
pill groups, and a label plus one short control — not two full-phrase controls
on a row.** Measured: 24 characters at Inter's 1.0202em worst case is ~383px
against 328px available, so `03-UI-SPEC.md`'s stacking rule keeps its practice
and loses only its stated reason (CD-1).

---

## The Ramp Strip (Phase 4, D4-01, plan 04-07)

**One contiguous band with ONE border, on the band.** A border per segment reads
as five swatches in a row, which is the "multi boxes" the redesign deletes. It
must visually rhyme with the exported legend bar — same idiom, one in chrome
tokens and one in composition ink. `border: 0` on each segment is load-bearing:
`theme.css` gives every `button` a hairline, so without it the band grows five
internal edges.

**`labelInkForShade(shade)` decides the check glyph AND the ramp contrast
gate.** `ramps.test.ts` Gate 3 used to inline `Math.max(onPaper, onInk)`, which
is a property of the **shade** rather than of the **choice** — it would stay
green if the renderer picked the other colour. One function, so the gate rates
the renderer's real pick.

**It takes only a shade, and that arity is the dark-mode guarantee.** A shade is
the product, not a token; it does not flip, so neither can its label. There is
no theme parameter to pass and therefore **nowhere for a `.dark` variant to
enter** — mode-invariance by construction rather than by a rule someone has to
remember. The strip authors zero per-component dark overrides, and
`colorPicker.css` contains no `.dark`.

**Hover paint on a palette is INSTANT, and the default is against you.** The
global `button` rule transitions `background-color`, so the segments inherit a
fade unless something cancels it. They join the rail rows' existing
`transition: none` group in `editor.css`, and the gate has two halves: the
surfaces must resolve to `transition: none`, **and** no rule may transition a
background on a selector reaching them. An ease between two ramp shades is a
regression, not polish.

**The focus ring is INSET, and the reason is geometric.** The global ring is an
`outline`, which paints *outside* the border box — and the band's
`overflow: hidden` clips it away at the first and last segment. `box-shadow:
inset` cannot be clipped. **A computed-style read is not this claim**: it would
report the outline as present in exactly the case where the creator cannot see
it, so the gate samples rendered pixels at each inner edge and carries its own
control (a near-white `Greys` shade, asserted to yield zero focus-blue pixels
while unfocused). **Reach the segment by KEYBOARD in the probe** — Chrome only
matches `:focus-visible` when focus arrived from the keyboard, so
`locator.focus()` produces a focused segment with no ring and a probe that
reports the defect on a correct implementation.

**Selection is carried by three things, because colour cannot carry it alone.**
`aria-pressed`, a 16px check glyph, and the `Step <i> of <n> · <HEX>` readout.
The measured minimum separation between neighbouring shades across the shipped
ramps is **1.2944:1**.

**No accent anywhere in the strip.** D-05 spends the Colors panel's Apple Blue
on `Apply Color`, so hover and selection are the same 2px Midnight Ink inset
ring — never a Powder background, which would hide the very shade being
selected. The accent budget is now **enumerated two-way** in
`uiContract.test.ts`: the shared ghost's properties are asserted, and the set of
rules filling from `--accent-fill` is pinned to a literal four-surface list.

**The strip's pure vocabulary lives in `ramps.ts`, not beside the component.**
Vitest runs on `node` with no DOM, so a format string declared in a `.tsx` file
cannot be gated at all — and an ungated `Step i of n` format is one copy-paste
from a second spelling in the legend. `rampStepPosition` **throws** outside the
strip where `shadeForValue` clamps: the latter takes a creator-supplied position
from stored data, the former takes an index this repository produced.

**`setColorValues` is the write seam for a colour picked by IDENTITY.**
`setColor`/`setColors` still speak hex because that is what a text field
produces; the strip hands the union straight through, so a ramp assignment is
never flattened to a hex and re-parsed. Both land on the same reducer action and
the same `hasEffectiveColorChange`, and there is still exactly one
`resolveColorValue` (Live Invariant 10).

**Assertion 27 now names TWO roving-tabindex owners, and it is still a SET.**
The defect it covers is an *unnamed* writer — two files disagreeing about which
element holds the single tab stop — not the existence of a second roving group.
`RampStrip.tsx` is one because A5 needs five segments to be one tab stop, and
the container-plus-`aria-activedescendant` alternative would put the focus ring
on the container and break the inset-ring requirement. A third still fails and
still has to say why. **The rail's rows remain plain tab stops.**

**`collectTabOrder` resolves form controls through `aria-labelledby` and their
associated `<label>`.** A radio inside a pill and a field named by its legend
have no child text of their own, so the walk used to report them as `''` — and
an order asserted against a run of empty strings cannot catch a swap.
