...
**Refusal, never truncation, and ONE derivation feeds both halves.** `characterBoundFor(role,
size)` decides the counter's `--destructive` state *and* the export refusal, so the number a
creator watches turn red can never disagree with the number that blocks the export. The length cap
(100 code points) is deliberately **above** the largest character bound (49): a creator has to be
able to overshoot and be told what to shorten, and a cap at the bound would silently clip.

**Vitest runs on `node`, so a bound is a character count.** It is derived from the repository's own
recorded measurement — `W` at **1.0202em**, the same number `LEGEND_CHARACTERS_PER_LINE` records.
`04-11` and `04-13` derive from that one measurement and **whoever moves it moves both**. It is
load-bearing rather than decorative: a naive 1.0-em assumption gives a medium title 23 characters
where the measurement gives 22, and a test that tried to *recover* the advance from
`LEGEND_CHARACTERS_PER_LINE` went green against an advance of `1` — `floor()` eats the difference
at all three legend sizes. The assertion that can fail names the difference instead.
