...
| 02-02-T1 | 02-02 / 1 | Task 1: Define the single canvas handle, effective-scene, and complete-composition contracts | F2.1, F2.2, F2.4, F3.1, F3.2, F3.3, F3.4, F3.5, F4.1, F4.2, F4.3, F4.4, F4.5, F6.1, F6.2, F7.1, F7.2, F7.3, NFR9, NFR11 | src/types/map.ts, src/types/composition.ts, src/types/ui.ts | `npm exec tsc -- -p tsconfig.app.json --noEmit && npm run lint` | pending |
...
| 02-12-T1 | 02-12 / 3 | Task 1: Validate readiness, durable source approvals, assets, and factual approvals | F2.1, F2.2, F2.4, F2.5, NFR3, NFR8, NFR9 | src/utils/historicalValidation.test.ts, src/utils/historicalValidation.ts | `npm test -- src/utils/historicalValidation.test.ts src/utils/scene.test.ts` | pending |
...
| 02-15-T1 | 02-15 / 6 | Task 1: Generate the 1815 candidate and review atlas | F2.1, F2.2, F2.3, F2.4, F2.5, NFR8, NFR9 | data/historical-reviewed/1815.geojson, data/historical-reviewed/1815.review.json, data/historical-reviewed/1815.review.html | `node scripts/prepareHistoricalSnapshot.mjs --snapshot 1815 --sources sources/historical/1815.sources.json --input sources/historical/1815.input.geojson --source-approval sources/historical/1815.source-approval.json --output data/historical-reviewed/1815.geojson --review-output data/historical-reviewed/1815.review.json --review-html data/historical-reviewed/1815.review.html` | pending |
...
| 02-33-T2 | 02-33 / 5 | Task 2: Materialize and validate four durable source-approval artifacts | F2.1, F2.3, F2.4, NFR8 | sources/historical/{1492,1700,1815,1914}.source-approval.json | `for id in 1492 1700 1815 1914; do node scripts/prepareHistoricalSnapshot.mjs --snapshot "$id" --sources "sources/historical/$id.sources.json" --input "sources/historical/$id.input.geojson" --source-approval "sources/historical/$id.source-approval.json" --validate-source-approval \|\| exit 1; done && npm test -- src/utils/historicalValidation.test.ts src/utils/historicalPreparationCli.test.ts` | pending |
...
- `src/utils/historicalValidation.test.ts` and `historicalPreparationCli.test.ts` — canonical archive/member inventories; vector/manual modes; durable source reviewer identity/role/date/non-executor status; six source decisions; mode-specific manual hashes; source-approval SHA chaining; factual hashes; stale-byte invalidation; offline checks.
- `sources/historical/{id}.source-approval.json` — durable source reviewer identity/role/date, explicit non-executor/non-implementer status, six decisions, source manifest/canonical archive/member inventory/input/mode/manual-trace hashes.
- `data/historical-reviewed/{id}.approval.json` — qualified factual reviewer name/role/date, six decisions/uncertainties, current source-approval SHA, and source/input/output/review JSON/review HTML hashes.
...
- `src/components/MapCanvas.test.tsx`, `src/App.test.tsx`, and transaction hook tests — exactly one controller-backed `MapCanvasHandle`, visible-SVG callbacks, responsive rebind, live save, atomic load, freeze settlement, outermost-finally release, success/failure/remount/post-release input.
- `src/components/MapWorkspace.test.tsx`, `src/utils/export.test.ts`, and export E2E — one canonical SVG, camera-then-legend layer order, camera-independent exact live legend, no sibling fallback, and retention of all visual wrapped geography.
...
|---|---|---|
| Source/license readiness | F2.1, F2.3, NFR8 | Named reviewer/role/date, explicit non-executor/non-implementer status, PASS for all 24 records, and durable source-approval JSON binding manifest/canonical archive/member inventory/input/mode plus manual evidence/procedure/operator/control-point hashes |
...
- Plans 02-31/32 assemble canonical source bundles; Plan 02-33 performs non-executor review then writes four machine-validated durable source approvals; Plans 02-13–16 refuse generation without current source approval and bind its SHA into candidate requests; Plan 02-34 obtains qualified factual review; Plan 02-35 seals durable approvals; Plan 02-17 promotes catalog last.
...
| RESEARCH/REVIEWS | Single live MapCanvasHandle, visible-SVG operations, responsive rebind, and read/freeze/settle/release transaction | 02-02, 02-07, 02-08, 02-09, 02-18, 02-23, 02-29, 02-30 | COVERED |
| RESEARCH/REVIEWS | Durable non-executor source approval binding canonical archive/member inventory, input/mode, and honest manual-trace hashes | 02-12, 02-31–02-33, 02-13–02-17, 02-27, 02-34, 02-35 | COVERED |
| RESEARCH/REVIEWS | Qualified factual approval bound to current source-approval SHA plus five candidate/review hashes | 02-12, 02-13–02-17, 02-27, 02-34, 02-35 | COVERED |
| RESEARCH/REVIEWS | Approved historical entity interaction policy | 02-02, 02-07, 02-09, 02-10, 02-18–02-23 | COVERED |
| RESEARCH/REVIEWS | React legend group inside the one canonical SVG, after/outside camera transform, retained exactly in export with no sibling fallback | 02-02, 02-11, 02-18, 02-21, 02-23, 02-30 | COVERED |
...
|---|---|
| Single live camera-controller ownership bridge | 02-02-T1; 02-07-T1/T2/T3; 02-08; 02-09; 02-18-T2; 02-23-T1/T2; 02-29-T1/T2; 02-30-T1 |
...
| Durable source/license approval bytes and downstream SHA chain | 02-12-T1/T2; 02-31; 02-32; 02-33-T1/T2; 02-13–02-17; 02-27; 02-34; 02-35 |
| Durable factual reviewer identity/six decisions/source-approval SHA/five hashes/no self-approval | 02-12-T1/T2; 02-34; 02-35; 02-17 |
...
| Legend inside canonical export SVG / no sibling fallback / exact clone retention | 02-11-T2; 02-18-T2; 02-21-T1/T2; 02-23-T1/T2; 02-30-T1 |
...
- [x] 36 plans and 72 tasks have a planned automated command and green completion state.
- [x] All 41 required ROADMAP requirement IDs are assigned.
- [x] D-01 through D-29 appear in actionable plan content.
- [x] Dependency graph remains acyclic across 18 waves; same-wave file ownership has no overlap, including Plan 02-33's four new source-approval artifacts.
...
**Approval:** targeted review revision complete — durable source approvals, sole MapCanvasHandle ownership, and canonical in-SVG legend are mapped exactly; execution pending
